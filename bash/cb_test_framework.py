...
class CBTester:
    ...
    def test_help_functionality(self):
        ...
        missing_sections = [s for s in expected_sections if s not in help_output]
        all_found = len(missing_sections) == 0
        ...
        test_result = TestResult(
            ...
            error_message=f"Missing sections: {missing_sections}" if not all_found else "",
        )
        self.results.append(test_result)
        status = "✅" if test_result.passed else "❌"
        print(f"   {status} Help contains all expected sections")

        # Test help text formatting (colors/escape sequences should not cause errors)
        self.test_case("Help text formatting works", ["-h"], 0, None, use_pty=True)

        # Test help in narrow terminal (should use simple format, no box drawing)
        ...
        self.results.append(narrow_result)
        status = "✅" if narrow_result.passed else "❌"
